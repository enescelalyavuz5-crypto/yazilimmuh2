...
export default function Words() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5050'}/v1/words?limit=20`)
      .then(res => res.json())
  ...
  useEffect(() => {
    if (!userId) return;
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5050'}/v1/words/favorites?userId=${userId}`)
      .then(res => res.json())
      .then(data => {
        if (data?.data) {
