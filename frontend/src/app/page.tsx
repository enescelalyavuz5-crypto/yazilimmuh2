...
export default function Home() {
  // 0: empty, 1: student typing, 2: student sent, 3: AI typing, 4: AI sent
  const [chatStep, setChatStep] = useState(0);

  useEffect(() => {
    const t1 = setTimeout(() => setChatStep(1), 800); // Wait then student starts typing
    const t2 = setTimeout(() => setChatStep(2), 2000); // Student sent message
    const t3 = setTimeout(() => setChatStep(3), 2500); // AI starts typing
    const t4 = setTimeout(() => setChatStep(4), 4500); // AI sends message
    ...
    <div className="flex flex-col space-y-32 pb-20">
      ...
      <section className="flex flex-col lg:flex-row items-center gap-12 mt-8 lg:mt-16">
        ...
        <div className="flex-1 w-full max-w-lg relative animate-in slide-in-from-right-8 duration-700">
          ...
          <div className="relative bg-[#0A0A0A] border border-white/10 rounded-3xl p-6 shadow-2xl flex flex-col gap-6 min-h-[400px]">
            ...
            <div className="flex items-center gap-3 border-b border-white/10 pb-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-400">{chatStep === 3 ? "Yazıyor..." : "Çevrimiçi"}</p>
              </div>
            </div>
            ...
            <div className="space-y-4 flex-1 flex flex-col justify-end">
              {chatStep >= 2 && (
                <div className="flex flex-col items-end gap-1 animate-in slide-in-from-bottom-2 fade-in duration-300">
              ...
              {chatStep === 3 && (
                <div className="flex flex-col items-start gap-1 animate-in fade-in duration-300">
                  <div className="bg-white/10 border border-white/5 px-4 py-4 rounded-2xl rounded-tl-sm w-auto shadow-md">
              ...
              {chatStep >= 4 && (
                <div className="flex flex-col items-start gap-1 animate-in slide-in-from-bottom-2 fade-in duration-500">
            ...
            <div className="mt-2 bg-white/5 border border-white/10 rounded-xl px-4 py-3 flex justify-between items-center text-neutral-500 text-sm">
              <span className="flex-1">
                {chatStep === 0 ? "" : chatStep === 1 ? (
                  <span className="border-r-2 border-emerald-500 pr-1 animate-pulse">I readed a bo</span>
                ) : "Mesajını yaz..."}
              </span>
              <svg className={`w-5 h-5 ${chatStep >= 2 ? "text-emerald-500" : "text-neutral-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
            </div>
          </div>
        </div>
      </section>
